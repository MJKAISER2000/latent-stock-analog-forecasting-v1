...
CONFIGS = [
    {
        "name": "no_sector_top5_inverse_vol",
        "top_n": 5,
        "inverse_vol": True,
    },
    {
        "name": "no_sector_top10_inverse_vol",
        "top_n": 10,
        "inverse_vol": True,
    },
    {
        "name": "no_sector_top20_inverse_vol",
        "top_n": 20,
        "inverse_vol": True,
    },
]
...
def load_predictions() -> pd.DataFrame:
    path = "outputs/tables/week19_no_sector_ranker_predictions_full500_1m.csv"
...
def main():
    ...
    for config in CONFIGS:
        ...
        baskets = build_baskets(pred, config["top_n"])
        ...
        stats = {
            "strategy": f"week19_{config['name']}",
            "config": config["name"],
            "top_n": config["top_n"],
            "transaction_cost": TRANSACTION_COST,
        ...
        baskets["strategy"] = f"week19_{config['name']}"
        holdings_rows.append(baskets)

    stats_df = pd.DataFrame(stats_rows)
    holdings_df = pd.concat(holdings_rows, ignore_index=True)

    stats_path = "outputs/tables/week19_no_sector_ranker_backtest_stats.csv"
    curves_path = "outputs/tables/week19_no_sector_ranker_backtest_curves.csv"
    holdings_path = "outputs/tables/week19_no_sector_ranker_backtest_holdings.csv"
    report_path = "outputs/reports/week19_no_sector_ranker_backtest_summary.txt"
    ...
    with open(report_path, "w", encoding="utf-8") as f:
        f.write("Week 19 No-Sector Ranker Backtest Summary\n")
        f.write("========================================\n\n")
        f.write("Goal:\n")
        f.write("Backtest the LightGBM ranker after removing sector and industry identity features.\n\n")
