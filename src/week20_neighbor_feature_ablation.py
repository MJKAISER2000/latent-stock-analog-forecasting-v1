...
DATASET_PATH = "data/processed/week20_full500_with_stock_latent_neighbors.parquet"
...
def get_feature_columns(df: pd.DataFrame, feature_set: str) -> list[str]:
    ...
    neighbor_cols = [c for c in all_features if c.startswith("neighbor_")]
    original_cols = [c for c in all_features if not c.startswith("neighbor_")]
    ...
    if feature_set == "original_plus_neighbors":
        return original_cols + neighbor_cols
...
def load_features_for_weights() -> pd.DataFrame:
    ...
    if "vol_12m" not in df.columns:
        raise ValueError("vol_12m missing from dataset.")
...
def train_predict_for_year(
    ...
    print("=" * 100)
    print(
        f"Neighbor ablation fold | feature_set={feature_set} | "
    ...
    out["model"] = "walk_forward_lgbm_ranker_neighbor_ablation"
...
def main():
    ...
    print("Loading dataset with stock latent neighbor features...")
    df = pd.read_parquet(DATASET_PATH)
    df["date"] = pd.to_datetime(df["date"])
    df["ticker"] = df["ticker"].astype(str).str.strip().str.upper()
    ...
    for feature_set in FEATURE_SETS:
        ...
        print("Neighbor feature count:", len(neighbor_cols))
        print("Neighbor features:", neighbor_cols)
        ...
        for top_n in TOP_NS:
            ...
            stats = {
                ...
                "strategy": f"week20_neighbor_ablation_{feature_set}_top{top_n}_inverse_vol",
                "top_n": top_n,
                "transaction_cost": TRANSACTION_COST,
            ...
            baskets["strategy"] = f"neighbor_ablation_{feature_set}_top{top_n}"
    ...
    stats_path = "outputs/tables/week20_neighbor_feature_ablation_stats.csv"
    predictions_path = "outputs/tables/week20_neighbor_feature_ablation_predictions.csv"
    curves_path = "outputs/tables/week20_neighbor_feature_ablation_curves.csv"
    holdings_path = "outputs/tables/week20_neighbor_feature_ablation_holdings.csv"
    report_path = "outputs/reports/week20_neighbor_feature_ablation_summary.txt"
    ...
    with open(report_path, "w", encoding="utf-8") as f:
        f.write("Week 20 Latent Neighbor Feature Ablation Summary\n")
        f.write("===============================================\n\n")
        f.write("Goal:\n")
        f.write(
            "Test whether stock-level latent nearest-neighbor outcome features improve walk-forward ranker performance.\n\n"
        )
    ...
    print("")
    print("NEIGHBOR FEATURE ABLATION RESULTS BY ANNUALIZED RETURN")
    print(stats_df.sort_values("annualized_return", ascending=False).to_string(index=False))

    print("")
    print("NEIGHBOR FEATURE ABLATION RESULTS BY SHARPE")
    print(stats_df.sort_values("sharpe_no_risk_free", ascending=False).to_string(index=False))

    print("")
    print("NEIGHBOR FEATURE ABLATION RESULTS BY RETURN / DRAWDOWN")
