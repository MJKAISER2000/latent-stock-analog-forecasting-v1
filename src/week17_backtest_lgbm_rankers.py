...
DATASET_PRICE_FILES = {
    "week15_full500": "data/processed/week15_500_monthly_prices.parquet",
    "week15_highvol100": "data/processed/week15_500_monthly_prices.parquet",
    "week15_highvol200": "data/processed/week15_500_monthly_prices.parquet",
    "week16_balanced450": "data/processed/week16_1000_monthly_prices.parquet",
    "week16_balanced900": "data/processed/week16_1000_monthly_prices.parquet",
}
...
def load_monthly_returns(price_path: str) -> pd.DataFrame:
...
def load_ranker_predictions(dataset_name: str, horizon: int) -> pd.DataFrame:
    path = f"outputs/tables/week17_lgbm_ranker_predictions_{dataset_name}_{horizon}m.csv"
...
def build_baskets(
    ...
    for date, group in merged.groupby("date"):
        ...
        selected = (
            g.sort_values("ranker_score", ascending=False)
...
def plot_top_curves(curves: pd.DataFrame, stats: pd.DataFrame, output_path: str):
    ...
    plt.title("Week 17 LightGBM Ranker Backtest")
    plt.xlabel("Date")
...
def main():
    ...
    for dataset_name in DATASET_PRICE_FILES:
        print("")
        print("=" * 100)
        print("Dataset:", dataset_name)

        monthly_returns = load_monthly_returns(DATASET_PRICE_FILES[dataset_name])
        features = load_features(DATASET_FEATURE_FILES[dataset_name])

        for config in STRATEGY_CONFIGS:
            horizon = config["horizon"]

            strategy_name = f"{dataset_name}_lgbm_ranker_{config['name']}"

            print("Backtesting:", strategy_name)

            pred = load_ranker_predictions(dataset_name, horizon)
    ...
    # Benchmark uses all available dates from merged curves.
    # Use Week 15 500 returns for a stable SPY/equal-weight reference.
    benchmark_returns = load_monthly_returns("data/processed/week15_500_monthly_prices.parquet")

    eq = build_equal_weight_benchmark(benchmark_returns, all_curves["date"])
    spy = build_spy_benchmark(benchmark_returns, all_curves["date"])
    ...
    stats_path = "outputs/tables/week17_lgbm_ranker_backtest_stats.csv"
    ...
    figure_path = "outputs/figures/week17_lgbm_ranker_backtest_equity_curves.png"
    report_path = "outputs/reports/week17_lgbm_ranker_backtest_summary.txt"
    ...
    with open(report_path, "w", encoding="utf-8") as f:
        f.write("Week 17 LightGBM Ranker Backtest Summary\n")
        f.write("=======================================\n\n")
        f.write("Goal:\n")
        f.write(
            "Backtest LightGBM LambdaRank predictions using realistic top-N portfolio construction and risk controls.\n\n"
        )
