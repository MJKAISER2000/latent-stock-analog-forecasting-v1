...
def download_prices(tickers: list[str], start: str = "2014-01-01") -> pd.DataFrame:
    ...
    for i, ticker in enumerate(tickers, start=1):
        ...
        try:
            ...
            # yfinance can sometimes return a DataFrame for one ticker.
            if isinstance(close, pd.DataFrame):
                close = close.iloc[:, 0]

            close.name = ticker

            # Require enough data for long-horizon tests.
...
def main():
    ...
    raw_path = "data/raw/week15_500_daily_prices.parquet"
    monthly_path = "data/processed/week15_500_monthly_prices.parquet"
    report_path = "outputs/reports/week15_500_price_download_summary.txt"
    ...
    lines = []
    lines.append("Week 15 500-Stock Price Download Summary")
    lines.append("========================================")
    lines.append("")
    lines.append("Goal:")
    lines.append("Download daily and monthly prices for the broad 500-stock universe.")
    lines.append("")
    ...
    lines.append("")
    lines.append("Usable tickers:")
    lines.append(", ".join(usable_tickers))
    lines.append("")
    ...
    lines.append(
        "This universe uses current S&P 500 constituents, so it is not survivorship-bias-free. "
        "It is being used as a broad stress-test universe for Week 15."
    )
