...
def train_predict_for_year(df: pd.DataFrame, feature_cols: list[str], prediction_year: int) -> pd.DataFrame:
    print("")
    print("=" * 100)
    print(f"Walk-forward fold: train before {prediction_year}, predict {prediction_year}")
    ...
    # Use last 20% of training dates as validation.
    ...
    out["model"] = "walk_forward_lgbm_ranker"
    out["best_iteration"] = ranker.best_iteration_
    out["ranker_score"] = scores
    out["rank_by_date"] = out.groupby("date")["ranker_score"].rank(ascending=False, method="first")
...
def main():
    ...
    pred_path = "outputs/tables/week19_walk_forward_ranker_predictions.csv"
    ...
    for top_n in TOP_NS:
        ...
        stats = {
            "strategy": f"week19_walk_forward_top{top_n}_inverse_vol",
            "top_n": top_n,
            "transaction_cost": TRANSACTION_COST,
        ...
        baskets["strategy"] = f"walk_forward_top{top_n}"
        holdings_rows.append(baskets)

    stats_df = pd.DataFrame(stats_rows)
    holdings_df = pd.concat(holdings_rows, ignore_index=True)

    stats_path = "outputs/tables/week19_walk_forward_ranker_stats.csv"
    ...
    report_path = "outputs/reports/week19_walk_forward_ranker_summary.txt"
    ...
    with open(report_path, "w", encoding="utf-8") as f:
        f.write("Week 19 Walk-Forward Ranker Summary\n")
        f.write("==================================\n\n")
        f.write("Goal:\n")
        f.write("Retrain the LightGBM ranker through time and predict one year forward at a time.\n\n")
        f.write("Stats:\n")
