...
def main():
    ...
    for top_n in TOP_NS:
        ...
        stats = {
            "top_n": top_n,
            "transaction_cost": TRANSACTION_COST,
            "strategy": f"week15_full500_lgbm_ranker_h1_top{top_n}_inverse_vol",
        ...
        temp_holdings["strategy"] = f"top{top_n}"
        holdings_rows.append(temp_holdings)

    stats_df = pd.DataFrame(stats_rows)
    holdings_df = pd.concat(holdings_rows, ignore_index=True)

    stats_path = "outputs/tables/week17_ranker_topn_stress_stats.csv"
    curves_path = "outputs/tables/week17_ranker_topn_stress_curves.csv"
    holdings_path = "outputs/tables/week17_ranker_topn_stress_holdings.csv"
    report_path = "outputs/reports/week17_ranker_topn_stress_summary.txt"
    ...
    with open(report_path, "w", encoding="utf-8") as f:
        f.write("Week 17 Ranker Top-N Diversification Stress Test\n")
        f.write("===============================================\n\n")
        f.write("Goal:\n")
        f.write("Test whether the LightGBM ranker signal survives when moving from top 5 to broader top-N portfolios.\n\n")
