...
OUTPUT_STATS_PATH = "outputs/tables/week20_neighbor_ensemble_stats.csv"
OUTPUT_CURVES_PATH = "outputs/tables/week20_neighbor_ensemble_curves.csv"
OUTPUT_REGIME_PATH = "outputs/tables/week20_neighbor_ensemble_regime_features.csv"
REPORT_PATH = "outputs/reports/week20_neighbor_ensemble_summary.txt"


ENSEMBLES = [
    {
        "name": "ensemble_50_original20_50_neighbor10",
        "original_col": "original_only_top20_return",
        "neighbor_col": "neighbor_only_top10_return",
        "original_weight": 0.50,
        "neighbor_weight": 0.50,
    },
    {
        "name": "ensemble_70_original20_30_neighbor10",
        "original_col": "original_only_top20_return",
        "neighbor_col": "neighbor_only_top10_return",
    ...
    {
        "name": "ensemble_30_original20_70_neighbor10",
        "original_col": "original_only_top20_return",
        "neighbor_col": "neighbor_only_top10_return",
    ...
    {
        "name": "ensemble_50_original20_50_neighbor5",
        "original_col": "original_only_top20_return",
        "neighbor_col": "neighbor_only_top5_return",
        "original_weight": 0.50,
        "neighbor_weight": 0.50,
    },
    {
        "name": "ensemble_50_original10_50_neighbor10",
        "original_col": "original_only_top10_return",
        "neighbor_col": "neighbor_only_top10_return",
        "original_weight": 0.50,
        "neighbor_weight": 0.50,
    },
...
def main():
    ...
    ensemble_curves = merged[["date"]].copy()

    # Build unfiltered ensemble returns.
    for ens in ENSEMBLES:
        original_col = ens["original_col"]
        neighbor_col = ens["neighbor_col"]

        if original_col not in merged.columns:
            print(f"Skipping ensemble {ens['name']}: missing {original_col}")
            continue

        if neighbor_col not in merged.columns:
            print(f"Skipping ensemble {ens['name']}: missing {neighbor_col}")
            continue

        ensemble_return = (
            ens["original_weight"] * merged[original_col]
            + ens["neighbor_weight"] * merged[neighbor_col]
        )

        ensemble_curves[f"{ens['name']}_return"] = ensemble_return
        ensemble_curves[f"{ens['name']}_cumulative"] = (1 + ensemble_return).cumprod()

    full = ensemble_curves.merge(regime, on="date", how="left")

    stats_rows = []
    final_curves = ensemble_curves[["date"]].copy()

    ensemble_return_cols = [
        c for c in ensemble_curves.columns
        if c.endswith("_return")
    ]

    for return_col in ensemble_return_cols:
        ensemble_name = return_col.replace("_return", "")
        ...
        for rule in REGIME_RULES:
            ...
            curve_name = f"{ensemble_name}_{rule['name']}"
            final_curves[f"{curve_name}_return"] = temp["filtered_return"]
            final_curves[f"{curve_name}_cumulative"] = (
    ...
    final_curves.to_csv(OUTPUT_CURVES_PATH, index=False)
    regime.to_csv(OUTPUT_REGIME_PATH, index=False)

    with open(REPORT_PATH, "w", encoding="utf-8") as f:
        f.write("Week 20 Original + Latent-Neighbor Ensemble Summary\n")
        f.write("==================================================\n\n")
        f.write("Goal:\n")
        f.write(
            "Blend original-feature ranker returns with stock latent-neighbor ranker returns at the portfolio level.\n\n"
        )
    ...
    print("")
    print("ENSEMBLE RESULTS — TOP BY RETURN / DRAWDOWN")
    print(
        stats_df.sort_values("return_over_abs_drawdown", ascending=False)
        .head(30)
        .to_string(index=False)
    ...
    print("")
    print("ENSEMBLE RESULTS — TOP BY ANNUALIZED RETURN")
    print(
        stats_df.sort_values("annualized_return", ascending=False)
        .head(30)
        .to_string(index=False)
    ...
    print("")
    print("ENSEMBLE RESULTS — TOP BY SHARPE")
    print(
        stats_df.sort_values("sharpe_no_risk_free", ascending=False)
        .head(30)
        .to_string(index=False)
