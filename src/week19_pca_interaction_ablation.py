...
DATASET_PATH = "data/processed/week19_full500_with_market_pca_interactions.parquet"
...
def get_feature_columns(df: pd.DataFrame, feature_set: str) -> list[str]:
    ...
    pca_cols = [
        ...
        if c.startswith("market_pca_")
        and not c.startswith("market_pca_cluster_")
    ]

    interaction_cols = [
        c for c in all_features
        if c.startswith("interaction__")
    ]

    original_cols = [
        c for c in all_features
        if not c.startswith("market_pca_")
        and not c.startswith("interaction__")
    ]
    ...
    if feature_set == "original_plus_pca_interactions":
        return original_cols + pca_cols + interaction_cols

    if feature_set == "pca_interactions_only":
        return pca_cols + interaction_cols
...
def train_predict_for_year(
    ...
    print("=" * 100)
    print(
        f"Interaction ablation fold | feature_set={feature_set} | "
    ...
    out["model"] = "walk_forward_lgbm_ranker_pca_interaction_ablation"
...
def main():
    ...
    print("Loading PCA interaction dataset...")
    df = pd.read_parquet(DATASET_PATH)
    df["date"] = pd.to_datetime(df["date"])
    df["ticker"] = df["ticker"].astype(str).str.strip().str.upper()
    ...
    for feature_set in FEATURE_SETS:
        ...
        if leaked_cols:
            raise ValueError(
                f"Leakage columns found in feature set {feature_set}: {leaked_cols}"
            )

        pca_cols = [c for c in feature_cols if c.startswith("market_pca_")]
        interaction_cols = [c for c in feature_cols if c.startswith("interaction__")]

        print("")
        print("#" * 100)
        print(f"FEATURE SET: {feature_set}")
        print("Feature count:", len(feature_cols))
        print("PCA feature count:", len(pca_cols))
        print("Interaction feature count:", len(interaction_cols))
        ...
        for top_n in TOP_NS:
            ...
            stats = {
                ...
                "strategy": f"week19_interaction_ablation_{feature_set}_top{top_n}_inverse_vol",
                "top_n": top_n,
                "transaction_cost": TRANSACTION_COST,
            ...
            baskets["strategy"] = f"interaction_ablation_{feature_set}_top{top_n}"
    ...
    stats_path = "outputs/tables/week19_pca_interaction_ablation_stats.csv"
    predictions_path = "outputs/tables/week19_pca_interaction_ablation_predictions.csv"
    curves_path = "outputs/tables/week19_pca_interaction_ablation_curves.csv"
    holdings_path = "outputs/tables/week19_pca_interaction_ablation_holdings.csv"
    report_path = "outputs/reports/week19_pca_interaction_ablation_summary.txt"
    ...
    with open(report_path, "w", encoding="utf-8") as f:
        f.write("Week 19 PCA Interaction Ablation Summary\n")
        f.write("=======================================\n\n")
        f.write("Goal:\n")
        f.write(
            "Test whether explicit stock-feature x latent-market-state interaction features improve walk-forward ranker performance.\n\n"
        )
        ...
        f.write(
            stats_df.sort_values("sharpe_no_risk_free", ascending=False).to_string(
                index=False
            )
        )
        f.write("\n")
