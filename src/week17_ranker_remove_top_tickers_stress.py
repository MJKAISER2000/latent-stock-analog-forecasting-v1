...
def get_top_selected_tickers() -> list[str]:
    freq_path = "outputs/tables/week17_ranker_diagnostics_ticker_frequency.csv"

    freq = pd.read_csv(freq_path)
    freq["ticker"] = freq["ticker"].astype(str).str.strip().str.upper()

    return freq.sort_values("times_selected", ascending=False)["ticker"].tolist()


def build_baskets(pred: pd.DataFrame, top_n: int, banned_tickers: set[str]) -> pd.DataFrame:
    rows = []

    for date, group in pred.groupby("date"):
        g = group[~group["ticker"].isin(banned_tickers)].copy()

        selected = g.sort_values("ranker_score", ascending=False).head(top_n)

        rows.append(
            {
                "signal_date": date,
                "top_n": top_n,
                "banned_tickers": ", ".join(sorted(banned_tickers)),
...
def main():
    ...
    top_selected = get_top_selected_tickers()
    ...
    for remove_count in REMOVE_COUNTS:
        banned = set(top_selected[:remove_count])

        print("")
        print("=" * 80)
        print(f"Testing remove_count={remove_count}")
        print("Banned:", ", ".join(sorted(banned)) if banned else "None")

        baskets = build_baskets(pred, TOP_N, banned)
        ...
        label = f"remove_top_{remove_count}"

        stats = {
            "remove_top_n_tickers": remove_count,
            "banned_tickers": ", ".join(top_selected[:remove_count]),
            "strategy": f"week15_full500_lgbm_ranker_h1_top5_inverse_vol_{label}",
        ...
        temp_holdings["remove_top_n_tickers"] = remove_count
        temp_holdings["strategy"] = label
        holdings_rows.append(temp_holdings)

    stats_df = pd.DataFrame(stats_rows)
    holdings_df = pd.concat(holdings_rows, ignore_index=True)

    stats_path = "outputs/tables/week17_ranker_remove_top_tickers_stress_stats.csv"
    curves_path = "outputs/tables/week17_ranker_remove_top_tickers_stress_curves.csv"
    holdings_path = "outputs/tables/week17_ranker_remove_top_tickers_stress_holdings.csv"
    report_path = "outputs/reports/week17_ranker_remove_top_tickers_stress_summary.txt"
    ...
    with open(report_path, "w", encoding="utf-8") as f:
        f.write("Week 17 Ranker Remove-Top-Tickers Stress Test\n")
        f.write("============================================\n\n")
        f.write("Goal:\n")
        f.write("Test whether the ranker is dominated by its most-selected tickers.\n\n")
