...
def main():
    ...
    # Drop rows without monthly target.
    df = df.dropna(
        subset=[
            "future_1m_return",
            "future_1m_spy_return",
    ...
    print(df["target_outperform_spy_1m"].value_counts(normalize=True))
    ...
    y_train = train["target_outperform_spy_1m"]

    X_val = val[feature_cols]
    y_val = val["target_outperform_spy_1m"]

    X_test = test[feature_cols]
    y_test = test["target_outperform_spy_1m"]
    ...
    for model_name, model in models.items():
        ...
        prediction_output["predicted_prob_outperform_1m"] = test_probs
        prediction_output["rank_by_date"] = prediction_output.groupby("date")[
            "predicted_prob_outperform_1m"
        ].rank(ascending=False, method="first")

        pred_path = f"outputs/tables/week12_next_month_predictions_{model_name}.csv"
    ...
    metrics_path = "outputs/tables/week12_next_month_baseline_metrics.csv"
    portfolio_path = "outputs/tables/week12_next_month_portfolio_signal.csv"
    report_path = "outputs/reports/week12_next_month_baseline_summary.txt"

    metrics.to_csv(metrics_path, index=False)
    portfolio.to_csv(portfolio_path, index=False)

    with open(report_path, "w", encoding="utf-8") as f:
        f.write("Week 12 Next-Month Baseline Summary\n")
        f.write("===================================\n\n")
        f.write("Target:\n")
        f.write("target_outperform_spy_1m\n\n")
