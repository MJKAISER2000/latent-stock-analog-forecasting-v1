...
def load_walk_forward_curves() -> pd.DataFrame:
    path = "outputs/tables/week19_walk_forward_ranker_curves.csv"
...
def main():
    ...
    curves = load_walk_forward_curves()
    prices = load_prices()
    ...
    for portfolio_name, return_col in TOP_RETURN_COLS.items():
        ...
        for rule in REGIME_RULES:
            ...
            filtered_curves[f"{curve_name}_cumulative"] = (1 + temp["filtered_return"]).cumprod()

    stats_df = pd.DataFrame(stats_rows)

    stats_path = "outputs/tables/week19_walk_forward_regime_filter_stats.csv"
    curves_path = "outputs/tables/week19_walk_forward_regime_filter_curves.csv"
    regime_path = "outputs/tables/week19_walk_forward_regime_features.csv"
    report_path = "outputs/reports/week19_walk_forward_regime_filter_summary.txt"
    ...
    with open(report_path, "w", encoding="utf-8") as f:
        f.write("Week 19 Walk-Forward Regime Filter Summary\n")
        f.write("=========================================\n\n")
        f.write("Goal:\n")
        f.write("Apply tech/growth regime filters to walk-forward ranker results.\n\n")
    ...
    print("")
    print("WALK-FORWARD + REGIME FILTER — TOP BY RETURN / DRAWDOWN")
    print(
        stats_df.sort_values("return_over_abs_drawdown", ascending=False)
        .head(25)
        .to_string(index=False)
    ...
    print("")
    print("WALK-FORWARD + REGIME FILTER — TOP BY ANNUALIZED RETURN")
    print(
        stats_df.sort_values("annualized_return", ascending=False)
        .head(25)
        .to_string(index=False)
    ...
    print("")
    print("WALK-FORWARD + REGIME FILTER — TOP BY SHARPE")
    print(
        stats_df.sort_values("sharpe_no_risk_free", ascending=False)
        .head(25)
        .to_string(index=False)
